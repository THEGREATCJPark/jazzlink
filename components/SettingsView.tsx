...
interface SettingsViewProps {
  ...
  authLoading: boolean;
  navigateToEditor: (profile: { type: 'musician' | 'venue' | 'team' | 'general', id: string }) => void;
  navigateToProfileCreation: () => void;
  toggleTheme: () => void;
  theme: string;
}

const SettingsView: React.FC<SettingsViewProps> = ({ currentUser, currentUserProfile, authLoading, navigateToEditor, navigateToProfileCreation, toggleTheme, theme }) => {
  const [contentProfile, setContentProfile] = useState<{type: 'musician' | 'venue' | 'team', id: string} | null>(null);
  ...
  useEffect(() => {
    const fetchContentProfile = async () => {
        if (currentUser && currentUserProfile && currentUserProfile.accountType && db) {
            ...
            let profileType: 'musician' | 'venue' | 'team' | null = null;
